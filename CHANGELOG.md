...
- Removed deprecated compatibility paths: `CacheDict.writer()`, `CacheDictWriter`, `Step.with_input()`, `DumperLoader`, old handler names, `metadata=` JSONL, legacy `Auto` (`optimized`/`pickled`), `torch.save` tensor entries, and implicit `Auto` pickle fallback (use `AutoPickle`). [#331]
