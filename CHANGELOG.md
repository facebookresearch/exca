...
- `steps`: `Parallel` primitive — run a fixed set of step variants over one shared item set under a single backend dispatch, each variant caching under its own identity.
