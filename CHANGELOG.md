...
- Cache files are no longer forced to `0o777` but follow your umask: set `umask 002`, and call `exca.utils.setup_shared_folder(folder)` once on a cache root shared with others (`infra.permissions` is now deprecated and ignored). [#324]
